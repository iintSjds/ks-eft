"""
run_ks_pbe.jl (sodium)
Stage 1: Run KS-DFT (SCF + bands) with PBE functional and save all data needed for QP correction.

BCC Na, Zion=9, [He] core (smallcore), GTH PBE PSP.
Semicore: 2s² 2p⁶ treated as valence. Conduction band = band 5 (3s).
Saves: sodium/na_pbe_ks_data.jld2

Usage: julia --project=. sodium/run_ks_pbe.jl
"""

if "--dry-run" in ARGS
    println("[dry-run] sodium/run_ks_pbe.jl")
    println("  Computes: Na SCF + bands (PBE GTH Zion=9, BCC)")
    println("  Saves: sodium/na_pbe_ks_data.jld2")
    exit(0)
end
...
model = model_PBE(lattice, [ElementPsp(:Na, psp)], positions;
                  temperature=0.001, smearing=DFTK.Smearing.FermiDirac())

Ecut = 40.0  # higher Ecut needed for semicore states
kgrid = [8, 8, 8]
...
println("\n=== Bands ===")
n_bands_calc = 8  # 9 valence electrons → 5 occupied bands, plus 3 empty
kld = 20u"bohr"
t1 = time()
...
# Save
outfile = joinpath(outdir, "na_pbe_ks_data.jld2")
jldsave(outfile;

"""
run_ks_pbe.jl (aluminum)
Stage 1: Run KS-DFT (SCF + bands) with PBE functional and save all data needed for QP correction.

FCC Al, Zion=3, [Ne] core, GTH PBE largecore PSP.
Saves: aluminum/al_pbe_ks_data.jld2

Usage: julia --project=. aluminum/run_ks_pbe.jl
"""

if "--dry-run" in ARGS
    println("[dry-run] aluminum/run_ks_pbe.jl")
    println("  Computes: Al SCF + bands (PBE GTH Zion=3, FCC)")
    println("  Saves: aluminum/al_pbe_ks_data.jld2")
    exit(0)
end
...
model = model_PBE(lattice, [ElementPsp(:Al, psp)], positions;
...
# Save
outfile = joinpath(outdir, "al_pbe_ks_data.jld2")
jldsave(outfile;

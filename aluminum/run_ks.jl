...
FCC Al, Zion=3, [Ne] core, GTH LDA largecore PSP.
Saves: aluminum/al_ks_data.jld2

Usage: julia --project=. aluminum/run_ks.jl
"""

if "--dry-run" in ARGS
    println("[dry-run] aluminum/run_ks.jl")
    println("  Computes: Al SCF + bands (GTH Zion=3, FCC)")
    println("  Saves: aluminum/al_ks_data.jld2")
    exit(0)
end
...
model = model_LDA(lattice, [ElementPsp(:Al, psp)], positions;
...
# Save
outfile = joinpath(outdir, "al_ks_data.jld2")
jldsave(outfile;

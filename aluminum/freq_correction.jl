...
Usage: julia --project=. aluminum/freq_correction.jl
Prerequisite: julia --project=. aluminum/run_ks.jl
"""

if "--dry-run" in ARGS
    println("[dry-run] aluminum/freq_correction.jl")
    println("  Loads: aluminum/al_ks_data.jld2")
    println("  Computes: atomic ΔSCF + form factors + coherent QP correction")
    println("  Saves: aluminum/al_qp_data.jld2")
    exit(0)
end
...
ks_file = joinpath(outdir, "al_ks_data.jld2")
...
outfile = joinpath(outdir, "al_qp_data.jld2")
jldsave(outfile;
...
open(joinpath(outdir, "al_summary.txt"), "w") do io
    println(io, "# Al EFT QP correction summary (coherent formula)")
    println(io, "# Generated by aluminum/freq_correction.jl")
    println(io)

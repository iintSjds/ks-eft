"""
run_ks.jl (lithium)
Stage 1: Run KS-DFT (SCF + bands) with GTH LDA Zion=1 PSP.

BCC Li, Zion=1, [He] core (1s² frozen), GTH LDA largecore PSP.
Saves: lithium/li_ks_data.jld2

Usage: julia --project=. lithium/run_ks.jl
"""

if "--dry-run" in ARGS
    println("[dry-run] lithium/run_ks.jl")
    println("  Computes: Li SCF + bands (GTH LDA Zion=1, BCC)")
    println("  Saves: lithium/li_ks_data.jld2")
    exit(0)
end
...
model = model_LDA(lattice, [ElementPsp(:Li, psp)], positions;
...
outfile = joinpath(outdir, "li_ks_data.jld2")
jldsave(outfile;

...
Diamond Si (FCC + 2-atom basis), Zion=4, [Ne] core, GTH LDA largecore PSP.
Saves: silicon/si_ks_data.jld2

Usage: julia --project=. silicon/run_ks.jl
"""

if "--dry-run" in ARGS
    println("[dry-run] silicon/run_ks.jl")
    println("  Computes: Si SCF + bands (GTH Zion=4, diamond cubic)")
    println("  Saves: silicon/si_ks_data.jld2")
    exit(0)
end
...
# Save
outfile = joinpath(outdir, "si_ks_data.jld2")
jldsave(outfile;

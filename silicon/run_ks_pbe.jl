"""
run_ks_pbe.jl (silicon)
Stage 1: Run KS-DFT (SCF + bands) with PBE functional and save all data.

Diamond Si (FCC + 2-atom basis), Zion=4, [Ne] core, GTH PBE largecore PSP.
Saves: silicon/si_pbe_ks_data.jld2

Usage: julia --project=. silicon/run_ks_pbe.jl
"""

if "--dry-run" in ARGS
    println("[dry-run] silicon/run_ks_pbe.jl")
    println("  Computes: Si SCF + bands (GTH PBE Zion=4, diamond cubic)")
    println("  Saves: silicon/si_pbe_ks_data.jld2")
    exit(0)
end
...
# Save
outfile = joinpath(outdir, "si_pbe_ks_data.jld2")
jldsave(outfile;
    ...
    Zion = Int(psp.Zion),
    element = "Si", Z_nuc = 14, Z_val = 4,
    structure = "diamond", a_bohr = a,
)
println("→ $outfile")

"""
run_ks_pbe.jl (magnesium)
Stage 1: Run KS-DFT (SCF + bands) with PBE functional and save all data needed for QP correction.

HCP Mg, Zion=2, [Ne] core, GTH PBE largecore PSP.
Saves: magnesium/mg_pbe_ks_data.jld2

Usage: julia --project=. magnesium/run_ks_pbe.jl
"""

if "--dry-run" in ARGS
    println("[dry-run] magnesium/run_ks_pbe.jl")
    println("  Computes: Mg SCF + bands (PBE GTH Zion=2, HCP)")
    println("  Saves: magnesium/mg_pbe_ks_data.jld2")
    exit(0)
end
...
# Save
outfile = joinpath(outdir, "mg_pbe_ks_data.jld2")
jldsave(outfile;

...
Usage: julia --project=. magnesium/freq_correction.jl
Prerequisite: julia --project=. magnesium/run_ks.jl
"""

if "--dry-run" in ARGS
    println("[dry-run] magnesium/freq_correction.jl")
    println("  Loads: magnesium/mg_ks_data.jld2")
    println("  Computes: atomic ΔSCF + form factors + coherent QP correction")
    println("  Saves: magnesium/mg_qp_data.jld2")
    exit(0)
end
...
ks_file = joinpath(outdir, "mg_ks_data.jld2")
...
outfile = joinpath(outdir, "mg_qp_data.jld2")
jldsave(outfile;
...
open(joinpath(outdir, "mg_summary.txt"), "w") do io
    println(io, "# Mg EFT QP correction summary (coherent formula)")
    println(io, "# Generated by magnesium/freq_correction.jl")
    println(io)
    @printf(io, "Z = %d, Z_val = %d\n", MG_Z_NUC, ks["Z_val"])
    @printf(io, "Core: [Ne] = 1s² 2s² 2p⁶ (10 electrons)\n")
    @printf(io, "Valence: 3s²\n")
    @printf(io, "Structure: HCP, a = %.3f Bohr, c = %.3f Bohr, c/a = %.3f\n",
            ks["a_bohr"], ks["c_bohr"], ks["ca_ratio"])
    println(io)

"""
run_ks_pbe.jl (potassium)
Stage 1: Run KS-DFT (SCF + bands) with PBE functional and save all data needed for QP correction.

BCC K, Zion=9 (smallcore, [Ne] core), GTH PBE PSP.
Valence: 3s² 3p⁶ 4s¹ (9 electrons → 5 occupied bands)
Saves: potassium/k_pbe_ks_data.jld2

Usage: julia --project=. potassium/run_ks_pbe.jl
"""

if "--dry-run" in ARGS
    println("[dry-run] potassium/run_ks_pbe.jl")
    println("  Computes: K SCF + bands (PBE GTH Zion=9, BCC)")
    println("  Saves: potassium/k_pbe_ks_data.jld2")
    exit(0)
end
...
model = model_PBE(lattice, [ElementPsp(:K, psp)], positions;
...
# Save
outfile = joinpath(outdir, "k_pbe_ks_data.jld2")
jldsave(outfile;

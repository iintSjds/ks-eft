...
BCC K, Zion=1, [Ar] core, GTH LDA largecore PSP.
Saves: potassium/k_ks_data.jld2

Usage: julia --project=. potassium/run_ks.jl
"""

if "--dry-run" in ARGS
    println("[dry-run] potassium/run_ks.jl")
    println("  Computes: K SCF + bands (GTH Zion=1, BCC)")
    println("  Saves: potassium/k_ks_data.jld2")
    exit(0)
end
...
model = model_LDA(lattice, [ElementPsp(:K, psp)], positions;
...
outfile = joinpath(outdir, "k_ks_data.jld2")
jldsave(outfile;

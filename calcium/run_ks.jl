...
FCC Ca, Zion=2, [Ar] core, HGH LDA PSP (DFTK built-in).
Saves: calcium/ca_ks_data.jld2

Usage: julia --project=. calcium/run_ks.jl
"""

if "--dry-run" in ARGS
    println("[dry-run] calcium/run_ks.jl")
    println("  Computes: Ca SCF + bands (HGH Zion=2, FCC)")
    println("  Saves: calcium/ca_ks_data.jld2")
    exit(0)
end
...
psp = load_psp("hgh/lda/ca-q2")
@printf("PSP: %s (Zion=%d, lmax=%d)\n", psp.identifier, psp.Zion, psp.lmax)

model = model_LDA(lattice, [ElementPsp(:Ca, psp)], positions;
...
# Save
outfile = joinpath(outdir, "ca_ks_data.jld2")
jldsave(outfile;

...
Ca core: 1s² 2s² 2p⁶ 3s² 3p⁶ ([Ar], 18 electrons)
Valence: 4s² (Z_val = 2)

Usage: julia --project=. calcium/freq_correction.jl
Prerequisite: julia --project=. calcium/run_ks.jl
"""

if "--dry-run" in ARGS
    println("[dry-run] calcium/freq_correction.jl")
    println("  Loads: calcium/ca_ks_data.jld2")
    println("  Computes: atomic ΔSCF + form factors + coherent QP correction")
    println("  Saves: calcium/ca_qp_data.jld2")
    exit(0)
end
...
ks_file = joinpath(outdir, "ca_ks_data.jld2")
...
outfile = joinpath(outdir, "ca_qp_data.jld2")
jldsave(outfile;
...
open(joinpath(outdir, "ca_summary.txt"), "w") do io
    println(io, "# Ca EFT QP correction summary (coherent formula)")
    println(io, "# Generated by calcium/freq_correction.jl")
    println(io)
    @printf(io, "Z = %d, Z_val = %d\n", CA_Z_NUC, ks["Z_val"])
    @printf(io, "Core: [Ar] = 1s² 2s² 2p⁶ 3s² 3p⁶ (18 electrons)\n")
    @printf(io, "Valence: 4s²\n")
    @printf(io, "Structure: FCC, a = %.3f Bohr\n", ks["a_bohr"])
    println(io)
